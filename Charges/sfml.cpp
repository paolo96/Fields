...
std::shared_ptr<CustomCharge> currentCustom = NULL;
Field mainField;
...
int main()
{
	...
	while (window.isOpen())
	{
		...
		while (window.pollEvent(event))
		{
            if (event.type == sf::Event::Closed){
                ...
                if(showing_pop_up){
                    ...
                    if(adding_charge){
                        if(shapingCustom && currentCustom!=NULL){
        ...
        if(inputing_custom)
            if(dragging_mouse){
                sf::Vector2i position = sf::Mouse::getPosition(window);
                if(((position.x-pop_up_rect.getPosition().x)>=0 && (position.x-pop_up_rect.getPosition().x)<=pop_up_rect.getSize().x) && ((position.y-pop_up_rect.getPosition().y)>=0 && (position.y-pop_up_rect.getPosition().y)<=pop_up_rect.getSize().y)){
...
void initializeConfig(){
    ...
    std::shared_ptr<Charge> firstCenterCharge(new Charge(CIRCLE_SHAPE, windowWidth/2, windowHeight/2, firstChargeSize, 1));
